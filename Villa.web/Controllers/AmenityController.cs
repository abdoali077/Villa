...
namespace Villla.Web.Controllers
{
    ...
    public class AmenityController : Controller
    {
        ...
        {
            var vm = new AmenityDto
            {
                VillaList = await _amenityService.GetVillaListAsync()
            };
        ...
        public async Task<IActionResult> Create(AmenityDto dto)
        {
            if (!ModelState.IsValid)
            {
                dto.VillaList = await _amenityService.GetVillaListAsync();
                return View(dto);
            }
        ...
        {
            var dto = await _amenityService.GetByIdAsync(id);
